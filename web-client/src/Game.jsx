      ...
      <>
        ...
        <Stage width={width} height={width}>
          <Layer>
            {game.terrain &&
              game.terrain.map(t => (
                <HexTile tileSize={tileSize} pos={t[0]} ty={t[1]} />
              ))}
          </Layer>
          <Layer>
            {game.structs &&
              game.structs.map(s => (
                <Structure struct={s} tileSize={tileSize}></Structure>
              ))}
          </Layer>
          <Layer>
            {game.resources &&
              game.resources.map(r => (
                <Resource resource={r} tileSize={tileSize}></Resource>
              ))}
          </Layer>
          <Layer>
            {game.bots && game.bots.map(e => Bot({ bot: e, tileSize }))}
          </Layer>
        </Stage>
      </>
    );
