...
function Node({ id, schema, descriptor }) {
    ...
    return (
        ...
        >
            <Rect fill={descriptor.color} opacity={0.5} width={120} height={50}></Rect>
            <Text
                fill="white"
                text={"[" + id + "] " + descriptor.prompt + (descriptor.value ? ": " + JSON.stringify(schema.values[id], null, 2) : "")}
            ></Text>
        </Group>
    );
}
                ...
                <Layer>
                    {Object.keys(schema.connections).map(id => {
                        ...
                        return schema.connections[id].map(id => {
                            ...
                            return <Arrow fill="white" stroke="white" points={[p.x, p.y, (p.x + p1.x) / 2, (p.y + p1.y) / 2, p1.x, p1.y]}></Arrow>;
                        });
                    })}
                </Layer>
